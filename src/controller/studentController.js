...
const createStudent = async function (req, res) {
    ...
    try {
        ...
        if (!isValid(subject)) {
            return res.status(400).send({ status: false, message: "subject must be Provided" });
        }
        ...
        if (!isValid(marks)) {
            return res.status(400).send({ status: false, message: "marks must be provided" })
        }

        if (! /^0*[1-9]\d*$/.test(marks)) {
            return res.status(400).send({ status: false, message: "enter a valid marks" })
        }


        if (!isValid(teacherName)) {
            return res.status(400).send({ status: false, message: "teacher name must be provided" })
        }

        const teacherByTeacherName = await teacherModel.findOne({ name: teacherName })

        if (!teacherByTeacherName) {
            return res.status(404).send({ status: false, message: `no teacher found by name: ${teacherName}` })
        }
